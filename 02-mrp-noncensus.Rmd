...
### Setting up example with an artificially nonrepresentative sample {.unnumbered #artificially-nonrepresentative-sample}
...
```{r}
# Biased sample of 5,000 that oversamples Democrats
...
for(i in 1:nrow(states_df)) {
  ...
  standard_estimates_vector_sub <- state_standard_epred_mat %*% k_filtered / sum(k_filtered)
  states_df$standard_mrp_estimate[i] <- mean(standard_estimates_vector_sub)
  states_df$standard_mrp_estimate_se[i] <- sd(standard_estimates_vector_sub)
  ...
  noncensus_estimates_vector_sub <- colSums(t(state_noncensus_epred_mat)*
                                          K_filtered)/colSums(K_filtered)
  states_df$noncensus_mrp_estimate[i] <- mean(noncensus_estimates_vector_sub)
  states_df$noncensus_mrp_estimate_se[i] <- sd(noncensus_estimates_vector_sub)
